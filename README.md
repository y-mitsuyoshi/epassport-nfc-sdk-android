...
### 2. AI OCR機能（カメラでのMRZ取得）のセットアップと確認方法
本アプリは、カメラを使用した AI OCR による MRZ（機械読取領域）の自動読み取り機能をサポートしています。実機でこの機能を確認するには、以下のセットアップが必要です。

#### ① APIキーの配置
1. プロジェクトのルートディレクトリにある `local.properties` ファイルを開きます。
2. 以下のプロパティを追加し、ご利用になる OpenAI などの APIキーを設定して保存します。
   ```properties
   AI_OCR_API_KEY=your_actual_openai_api_key_here
   ```
   > [!NOTE]
   > `local.properties` はGit管理対象外（`.gitignore` に設定済み）のため、リポジトリにAPIキーが漏洩する心配はありません。

#### ② ビルドと動作確認
1. Android Studio でアプリをビルド＆実行し、実機にインストールします。
2. アプリ画面上で「Camera Mode（カメラモード）」を選択します。
3. カメラの権限（パーミッション）要求が表示されるので、許可します。
4. パスポートの顔写真ページ下部にある **MRZ（2行の機械読取領域）** をカメラのガイド枠内に収めます。
5. OCRによってMRZ（旅券番号、生年月日、有効期限）が自動で解析され、読み込み完了後にNFCスキャン待ち受け状態へと遷移します。

#### ③ システムの仕組み（APIキーの注入ロジック）
* **Gradleでの自動注入**: `:app` モジュールの `build.gradle.kts` がビルド時に `local.properties` から `AI_OCR_API_KEY` の値を読み取ります。これが自動生成される `BuildConfig.AI_OCR_API_KEY` フィールドに注入されます。
* **クライアント初期化**: `MainActivity.kt` 内で `AiOcrConfig.forLocalTesting` を使用して `BuildConfig.AI_OCR_API_KEY` を適用し、`AiOcrClient` (OpenAI等のベンダー向け) を生成して `CameraMrzScanner` に渡しています。

> [!IMPORTANT]
> **本番運用におけるセキュリティベストプラクティス**:
> ビルド時にAPIキーを `BuildConfig` にハードコードする構成は開発・テスト用です。本番運用では `SecureApiKeyProvider` を実装し、動的なサーバー配信や暗号化されたキーコンテナから安全にAPIキーを取得する構成に切り替えてください。

### 3. 実機でのNFC読み取りテスト
* OCRが完了する（あるいは「Manual Mode（手動モード）」で手動でMRZ情報を入力する）と、NFCスキャンが有効になります。
